...
**Date:** January 13, 2026**Repository:** Shamrock2245/shamrock-bail-portal-site**Commit:** 858d66a
...
1. Navigate to **Pages** → **FloridaCounties (Item )** (dynamic page)

1. Click **Page Settings** (gear icon)

1. Go to **Advanced Settings** → **Custom Code**
...
1. Click **Code** (</> icon) to open Velo code panel

1. Locate the page code file (e.g., `FloridaCounties (Item).js`)

1. **Backup existing code** (copy to a text file)

1. Replace with contents of either:
...
1. Find and open **Emergency CTA Lightbox** (or similar name)

#### 3.2 Update Lightbox Code

1. Click **Code** (</> icon)

1. Locate the lightbox code file

1. **Backup existing code**

1. Replace with contents of `src/lightboxes/EmergencyCtaLightbox.js`
...
1. Navigate to a county page (e.g., `/county/lee`)
...
1. Wait 30 seconds (or configured trigger time)
...
1. Verify CSS file URLs are correct

1. Try copying CSS directly into Wix Custom Code instead of linking

1. Clear browser cache and hard refresh (Ctrl+Shift+R or Cmd+Shift+R)

1. Check Wix site settings for any CSP (Content Security Policy) restrictions
...
1. Verify element IDs in code match element IDs in Wix Editor

1. Check that all required backend modules are deployed (e.g., `counties.jsw`)

1. Ensure Wix Data collections exist (e.g., `FloridaCounties`)

1. Test in Wix Preview mode first before publishing
...
1. Check its ID in the Properties panel

1. Update element IDs to match code expectations

1. Or update code to match existing element IDs

1. Refer to `IMPLEMENTATION_SUMMARY.md` for expected element IDs
...
1. Check that Wix mobile editor settings aren't overriding CSS

1. Test in actual mobile browser, not just desktop responsive mode

1. Verify viewport meta tag is present in page head

1. Check for conflicting CSS from Wix themes
...
1. Remove the newly added CSS links/code

1. Restore previous CSS if backed up
...
1. Restore the backed-up code you saved before replacement
...
1. Re-deploy the previous version to Wix
...
- **GitHub:** [https://github.com/Shamrock2245/shamrock-bail-portal-site](https://github.com/Shamrock2245/shamrock-bail-portal-site)
...
1. Verify all element IDs match between code and Wix Editor

1. Test in Wix Preview mode before publishing

1. Refer to project documentation in `docs/` folder

---

**Status:** ✅ Ready for deployment**Last Updated:** January 13, 2026**Next Steps:** Follow Step 1 to begin deployment
