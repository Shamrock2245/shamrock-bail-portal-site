...
function saveSigningLinksToWixBatch(signers, caseData) {
    ...
    return batchSaveToWixPortal_Server(documents);
}

/**
 * Server-side handler for batch portal sync
 * Accepts pre-formatted documents array (from frontend or internal)
 */
function batchSaveToWixPortal_Server(documents) {
    ...
    if (!documents || !Array.isArray(documents)) {
        return { success: false, message: 'Invalid documents payload' };
    }
    ...
    Logger.log(`🔄 Syncing ${documents.length} signing links to Wix (Server)...`);
