...
- **[2026-03-08] (Scraper: Charlotte & Hendry)** - Completely rewrote Charlotte and Hendry scrapers. Migrated from DrissionPage to `python + curl_cffi` and `BeautifulSoup` to bypass TLS fingerprinting blocks and resolve macOS subprocess compatibility issues. Successfully extracting full charges and bond amounts.
- **[2026-03-08] (Backend: GAS)** - Built and deployed complete Agency Management automation: Liability Reporting, Agent Commissions, Court Reminders, Client Check-Ins, and Payment Reconciliations. Synced code and exposed Time-Driven Trigger APIs for dashboard integration.
