 ...
 * Manages multi-turn WhatsApp conversations to collect complete bail bond data
 * 
 * FLOW:
 ...
 * @param {string} userName - User's name from WhatsApp profile
...
function processIntakeConversation(from, message, userName) {
  ...
  if (response.triggerDocuments) {
    try {
      const docResult = triggerDocumentGenerationFromWhatsApp(state.data, from);
...
/**
 * Trigger document generation from WhatsApp conversation data
 * This calls the existing generateAndSendWithWixPortal function
 */
function triggerDocumentGenerationFromWhatsApp(conversationData, phoneNumber) {
  console.log('Triggering document generation from WhatsApp intake...');
  ...
  const formData = {
    ...
    // Signing method: WhatsApp (tells system to send links via WhatsApp)
