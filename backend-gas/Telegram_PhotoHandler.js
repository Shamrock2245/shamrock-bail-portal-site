 ...
 * 
 * Handles photo uploads from Telegram for ID verification
 * Manages:
 * 1. Media download from Telegram Bot API
...
/**
 * Handle incoming photo from Telegram
 * @param {string} from - User's chat ID
...
function handlePhotoUpload(from, mediaId, mimeType, caption) {
  console.log(`Photo upload from ${from} (telegram): mediaId=${mediaId}, type=${mimeType}`);
  ...
  try {
    ...
    const photoBlob = telegram.downloadFile(mediaId);
...
/**
 * Request photo upload via Telegram
 * Called after document signing is complete
 */
function requestPhotoUpload(chatId, caseNumber) {
  const telegram = new TelegramBotAPI();

  // Initialize photo upload state
  const state = getPhotoUploadState(chatId);
  state.caseNumber = caseNumber;
  updatePhotoUploadState(chatId, state);
...
Just tap the paperclip/camera icon 📷 in Telegram and send them one by one.
