 ...
 * 
 * Provides secure one-time password authentication via WhatsApp.
 * Works alongside existing email magic link authentication.
 * 
 * Flow:
 * 1. User enters WhatsApp number on portal
 ...
 */
function WA_sendOTP(phoneNumber) {
    try {
        ...
        const cacheKey = `whatsapp_otp:${formattedPhone}`;
        const otpData = {
            code: otp,
        ...
        const result = whatsapp.sendAuthenticationOTP(formattedPhone, otp);
        ...
        if (result.success) {
            ...
            try {
                NotificationService.notifySlack('SLACK_WEBHOOK_ALERTS', {
                    text: `📱 WhatsApp OTP sent to ${maskPhoneNumber(formattedPhone)}`
                });
            } catch (e) {
        ...
        console.error('❌ Error in WA_sendOTP:', error);
        return {
            success: false,
 ...
 */
function WA_validateOTP(phoneNumber, otpCode) {
    try {
        // Validate inputs
        ...
        try {
            NotificationService.notifySlack('SLACK_WEBHOOK_ALERTS', {
                text: `✅ WhatsApp login successful: ${maskPhoneNumber(formattedPhone)} (${user.role})`
            });
        } catch (e) {
            console.warn('Could not send Slack notification:', e);
        }
        ...
        console.error('❌ Error in WA_validateOTP:', error);
        return {
            valid: false,
...
function findOrCreateUserByPhone(phoneNumber) {
    try {
        ...
        const email = `${phoneNumber.replace(/\D/g, '')}@whatsapp.shamrock.local`;
        const role = 'indemnitor'; // Default role for WhatsApp users
 ...
 */
function WA_resendOTP(phoneNumber) {
    try {
        const formattedPhone = formatPhoneNumber(phoneNumber);
        const cache = CacheService.getScriptCache();
        ...
        // Send new OTP
        return WA_sendOTP(phoneNumber);
        
    } catch (error) {
        console.error('❌ Error in WA_resendOTP:', error);
        return {
            success: false,
            message: 'System error. Please try again.'
        };
