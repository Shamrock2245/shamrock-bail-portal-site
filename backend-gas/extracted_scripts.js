...
// --- Script Block 5 ---
        ...
        <div class="entity-card charge" id="charge-${chargeCount}">
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input type="text" id="charge-${chargeCount}-power" placeholder="OSI3-4591234" value="${data.powerNumber || ''}" oninput="updateDocumentCount()">
            </div>
            <div class="form-group">
      ...
      `;
        ...
        function updateDocumentCount() {
            ...
            const chargeCards = document.querySelectorAll('.entity-card.charge');

            if (chargeCards.length === 0) {
                totalBondsCount = 0;
            } else {
                document.querySelectorAll('[id^="charge-"][id$="-power"]').forEach(input => {
                    const powers = input.value.split(/[,;\\s]+/).filter(Boolean);
                    totalBondsCount += (powers.length > 0 ? powers.length : 1);
                });
            }

            document.getElementById('selected-doc-count').textContent = checked;
            document.getElementById('appearance-bond-count').textContent = `${totalBondsCount} bonds`;
            document.getElementById('estimated-pages').textContent = `~${checked * 1.5 + totalBondsCount}`;
        ...
        function importBookingData(dataString) {
            try {
                ...
                // Fill Indemnitor if present
                if (getValue('indemnitorFullName', 'indemnitorFirstName', 'indemnitorLastName', 'indemnitorPhone', 'indemnitorEmail')) {
                    const mappedIndemnitor = {
                        firstName: getValue('indemnitorFirstName'),
                        lastName: getValue('indemnitorLastName'),
                        middleName: '',
                        relationship: getValue('indemnitorRelation', 'relation'),
                        dob: getValue('indemnitorDOB'),
                        phone: getValue('indemnitorPhone', 'phone'),
                        email: getValue('indemnitorEmail', 'email'),
                        address: getValue('indemnitorAddress'),
                        city: getValue('indemnitorCity'),
                        state: getValue('indemnitorState') || 'FL',
                        zip: getValue('indemnitorZip'),
                        employer: getValue('indemnitorEmployerName', 'employer'),
                        employerPhone: '',
                        employerCity: '',
                        employerState: '',
                        supervisor: getValue('indemnitorJobTitle', 'jobTitle'),
                        // References
                        ref1Name: getValue('reference1Name', 'ref1Name'),
                        ref1Relation: getValue('reference1Relation', 'ref1Relation'),
                        ref1Phone: getValue('reference1Phone', 'ref1Phone'),
                        ref1Address: getValue('reference1Address', 'ref1Address'),
                        ref2Name: getValue('reference2Name', 'ref2Name'),
                        ref2Relation: getValue('reference2Relation', 'ref2Relation'),
                        ref2Phone: getValue('reference2Phone', 'ref2Phone'),
                        ref2Address: getValue('reference2Address', 'ref2Address')
                    };

                    addIndemnitor(mappedIndemnitor);
                }
        ...
        async function sendSignNowInvite(documentId, formData, method = 'email') {
            ...
            if (method === 'sms') {
                ...
                const smsResult = await callGAS('sendSmsInvite', {
                ...
                if (smsResult.success) {
                    ...
                    callGAS('notifyPaymentLink', {
                        indemnitorPhone: indemnitorPhone,
                        indemnitorEmail: indemnitorEmail,
                        defendantName: defendantName
                    }).catch(e => console.error("Failed to send explicit payment link notification:", e));
                }
 ...
 */
        ...
        /**
         * Fill PDFs with form data using Adobe PDF Services API (via backend-gas)
         */
        async function fillPDFsWithData(pdfDocs, formData) {
            console.log(`Starting to fill ${pdfDocs.length} documents via Adobe API...`);
            ...
            // Execute all PDF filling requests to Adobe API in parallel
            const fillPromises = pdfDocs.map(async (doc) => {
                try {
                    console.log(`Filling ${doc.key} via Adobe PDF Services API...`);
                    ...
                    if (!response || !response.success) {
                        throw new Error(`Adobe API filling failed for ${doc.key}: ${response ? response.error : 'Unknown error'}`);
                    }

                    // Decode base64 to Uint8Array for merging
                    const binaryStr = atob(response.pdfBase64);
                    const bytes = new Uint8Array(binaryStr.length);
                    for (let i = 0; i < binaryStr.length; i++) {
                        bytes[i] = binaryStr.charCodeAt(i);
                    }

                    // Parse with pdf-lib to get pageCount for signature offset mapping
                    const { PDFDocument } = PDFLib;
                    const pdfDoc = await PDFDocument.load(bytes);
                    const pageCount = pdfDoc.getPageCount();

                    console.log(`  Completed ${doc.key} via Adobe API. Pages: ${pageCount}`);

                    return {
                        key: doc.key,
                        name: doc.name,
                        bytes: bytes, // for mergePDFs
                        data: response.pdfBase64, // for legacy base64 support
                        pageCount: pageCount,
                        isCopy: doc.isCopy,
                        indemnitorIndex: doc.indemnitorIndex
                    };
                } catch (err) {
                    console.error(`Error filling PDF ${doc.name} via Adobe:`, err);
                    // Return original unfilled document as fallback in case of failure
                    return doc;
                }
            });

            // Wait for all Adobe API jobs to complete
            const results = await Promise.all(fillPromises);
            filledPdfs.push(...results);
      ...
      `);
            ...
            return new Promise((resolve, reject) => {
                ...
                google.script.run
                    .withSuccessHandler(response => {
                        ...
                        const results = selectedDocs.map((doc) => {
                            const data = response[doc.key];
                            if (!data) {
                                console.warn(`Missing template data for ${doc.key}`);
                            }
                            return {
                                key: doc.key,
                                name: doc.name,
                                bytes: data ? Uint8Array.from(atob(data), c => c.charCodeAt(0)) : new Uint8Array(),
                                isCopy: doc.isCopy || false,
                                indemnitorIndex: doc.indemnitorIndex
                            };
                        });
                        resolve(results);
        ...
        async function uploadPDFToTempStorage(pdfBytes, formData) {
            return new Promise((resolve, reject) => {
                // Ensure safe base64 encoding without hitting call stack limits
                let binary = '';
                ...
                google.script.run
                    .withSuccessHandler(resolve)
                    .withFailureHandler(reject)
                    .saveCompositePDF(base64, filename);
            });
        }
...
// --- Script Block 6 ---
        ...
        var SocialHub = (function () {
            ...
            return {
                ...
                loadCredentialStatus: function () {
                    var grid = document.getElementById('social-cred-status-grid');
                    if (!grid) return;
                    grid.innerHTML = '<span style="color:var(--muted); grid-column:1/-1;">Checking credentials...</span>';
                    google.script.run
                        .withSuccessHandler(function (status) {
                            if (!status) {
                                grid.innerHTML = '<span style="color:var(--error); grid-column:1/-1;">Could not load credential status.</span>';
                                return;
                            }
                            var PLATFORM_LABELS = {
                                twitter: 'X (Twitter)', linkedin: 'LinkedIn', gbp: 'Google Business',
                                tiktok: 'TikTok', youtube: 'YouTube', facebook: 'Facebook',
                                instagram: 'Instagram', telegram: 'Telegram', threads: 'Threads',
                                skool: 'Skool', patreon: 'Patreon'
                            };
                            var html = '';
                            Object.keys(status).forEach(function (p) {
                                ...
                                var tag = ready
                                    ? '<span style="color:var(--success); font-size:10px;">Ready</span>'
                                    : '<span style="color:var(--warning); font-size:10px;">Pending</span>';
                                html += '<span style="display:flex; align-items:center; gap:5px;">' + dot + ' ' + label + ' ' + tag + '</span>';
                            });
                            if (!html) html = '<span style="color:var(--muted); grid-column:1/-1;">No credential data returned.</span>';
                            grid.innerHTML = html;
                        })
                        .withFailureHandler(function (err) {
                            if (grid) grid.innerHTML = '<span style="color:var(--muted); grid-column:1/-1;">Credential check unavailable.</span>';
                            console.warn('getSocialCredentialStatus failed:', err.message);
                        })
        ...
        const ShamrockApp = (function () {
            ...
            <tr>
                ...
                renderAttachments: function (data) {
                    ...
                    // 1. Check for Telegram Documents safely
            ...
            const api = {
                ...
                init: function () {
                    ...
                    // Load Social Hub credential status on startup
                    setTimeout(function () {
                        if (typeof SocialHub !== 'undefined' && SocialHub.loadCredentialStatus) {
                            SocialHub.loadCredentialStatus();
                        }
                    }, 1500);
