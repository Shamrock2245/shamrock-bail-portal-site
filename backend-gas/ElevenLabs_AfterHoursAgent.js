...
var AFTER_HOURS_AGENT_CONFIG = {
    ...
    systemPrompt: [
        ...
        "ID: Ask for a government ID. Give a choice: text a photo link, or email the photos. The moment they pick one, call request_id_photo. method is upload or email. For upload pass {{caller_phone}}. For email pass their email. Never email the jail. Do not say you sent anything until request_id_photo returns success. Then stay on the line. Tell them to photograph the front, then the back. Do not hang up. Do not email paperwork yet. When they say they uploaded it, or after a short wait, call check_id_upload with the same case_reference. If it is not there, wait and check again. When it is there, read back the name and address from the ID and get a yes. That ID fills the paperwork. Then call email_paperwork_to_indemnitor.",
        ...
        "check_id_upload: call this while they photograph the ID. If received is false, stay with them. If true, read the name back, then send paperwork.",
