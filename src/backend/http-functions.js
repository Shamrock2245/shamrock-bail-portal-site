...
import { generatePDF } from 'backend/pdf-generator';

export async function post_submitBailBondApplication(request) {
  try {
    const formData = await request.body.json();
    await wixData.insert("BailBondApplications", formData);
    const pdfUrl = await generatePDF(formData);
    return ok(pdfUrl);
  } catch (error) {
    console.error(error);
    return serverError({ message: "Error processing application" });
  }
}
